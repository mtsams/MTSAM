...
class MVadpter(nn.Module):
    def __init__(self, intc=3, dim=32):
        super(MVadpter, self).__init__()

        self.fc0 = nn.Linear(intc, intc // 8)

        # ----- 深度可分离卷积 (dilation=3) -----
        self.conv3 = nn.Conv3d(intc // 8, intc // 8,
                               kernel_size=3, padding=3, dilation=3,
                               groups=intc // 8, bias=True)  # groups=输入通道数，模拟深度可分离卷积的感受野
        self.conv5 = nn.Conv3d(intc // 8, intc // 8,
                               kernel_size=5, padding=6, dilation=3,
                               groups=intc // 8, bias=True)
        self.conv7 = nn.Conv3d(intc // 8, intc // 8,
                               kernel_size=7, padding=9, dilation=3,
                               groups=intc // 8, bias=True)

        # ----- 普通分组卷积（保持原dilation=1配置） -----
        self.conv33 = nn.Conv3d(intc // 8, intc // 8,
                                kernel_size=3, padding=1, dilation=1,
                                groups=intc // 8, bias=True)
        self.conv55 = nn.Conv3d(intc // 8, intc // 8,
                                kernel_size=5, padding=2, dilation=1,
                                groups=intc // 8, bias=True)
        self.conv77 = nn.Conv3d(intc // 8, intc // 8,
                                kernel_size=7, padding=3, dilation=1,
                                groups=intc // 8, bias=True)

        # 1*1*1 conv 调整通道数
        self.conv_1x1x1 = nn.Conv3d(intc // 8, dim, kernel_size=1)
        self.bat = nn.BatchNorm3d(intc // 8)
    ...
    def forward(self, x, y):
        B, D, H, W, C = x.shape
        x=self.drop(x)
        x_pool = x.mean(dim=(1, 2, 3))  # (B, intc)
        x_vec = self.fc0(x_pool)  # (B, C_red)
        # 2) y → (B, C_red)
        y_vec = self.drop(y)
        y_vec = self.fc_y(y_vec)  # (B, C_red)
        z_vec = self.bilinear(x_vec, y_vec)  # (B, C_red)
        z = z_vec.view(B, 1, 1, 1, -1)  # (B,1,1,1,C_red)
        z=self.sig(z)
        x=self.fc_x(x)
        x = x * z
        ...
        feature_3 = self.relu(self.conv3(x))
        feature_3 = self.conv33(feature_3)

        feature_5 = self.relu(self.conv5(x))
        feature_5 = self.conv55(feature_5)

        feature_7 = self.relu(self.conv7(x))
        feature_7 = self.conv77(feature_7)

        combined_feature = feature_3 + feature_5 + feature_7
        combined_feature = self.bat(combined_feature)
        combined_feature = self.relu(combined_feature)
        combined_feature = self.drop(combined_feature)

        x = self.conv_1x1x1(combined_feature)
...
class Attention(nn.Module):
    ...
    def __init__(
        ...
        self.adapter = MVadpter(intc=dim, dim=dim*3)
    ...
    def forward(self, x,z):
        ...
        adapter=self.adapter(x,z)
        qkv_original = self.qkv(x)

        qkv = qkv_original + adapter
